...
from switchbot_cloud import Device, SwitchBot
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def async_update_data() -> dict[str, Device]:
        try:
            ...
            devices = [
                d
                for d in await hass.async_add_executor_job(switchbot.devices)
                if d.cloud_enabled
            ]
            ...
            raise UpdateFailed(
                f"Error communicating with the SwitchBot API: {err}"
            ) from err

    # Create a coordinator that can be used across all platforms.
    coordinator: SwitchBotDataUpdateCoordinator = hass.data[DOMAIN].setdefault(
        DATA_COORDINATOR,
